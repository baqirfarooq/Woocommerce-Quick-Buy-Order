...
class WooCommerce_Quick_Buy_Settings extends WC_Settings_Page {
	...
	public function __construct() {
		...
		$this->id    = 'wc_quick_buy_order_settings';
		$this->label = __( 'WC Quick Buy Order', WCQB_TXT );
    ...
    public function get_sections() {
        $sections = array(
            ''                    => __( 'General', WCQB_TXT ),
            'quick_buy_form'       => __('Quick Buy Form', WCQB_TXT ), 
	...
	public function get_settings($section = null) { 
		...
		} else if($section == 'button_style'){
			return $this->get_button_style();
		} else if($section == 'button_position'){
			return $this->get_button_position();
		} else if($section == 'quick_buy_form'){
			return $this->get_quick_buy_form();
		}  else {
	...
	public function get_general(){
		...
		$settings_array[] = array( 'name' => __( 'General Settings', WCQB_TXT ),'type' => 'title', 'desc' => __( 'The following options are used to configure WC Quick Buy Order Actions',WCQB_TXT ), 
	...
	public function get_button_style(){
		...
		$settings_array[] = array( 'name' => __( 'Quick Buy Button Settings', WCQB_TXT ),'type' => 'title', 'desc' => __( 'The following options are used to configure WC Quick Buy Order Button',WCQB_TXT ), 
	...
	public function get_button_position(){
		...
		$settings_array[] = array('name' => __( 'Single Product Page', WCQB_TXT ),
								  'type' => 'title', 
		...
		$settings_array[] = array('name' => __( 'Shop Page', WCQB_TXT ),
								  'type' => 'title', 
	...
	public function get_quick_buy_form(){
		...
		$settings_array[] = array('name' => __( 'Single Product Page', WCQB_TXT ),
								  'type' => 'title', 
								  'desc' => __( 'Options To Configure The Button In Single Product Page',WCQB_TXT ), 
            'id' => WCQB_DB.'quick_buy_form' );


	/*	$settings_array[] = array(
			'name' => __( 'Auto Add Quick Buy Form ', WCQB_TXT ),
			'desc_tip' => __( 'Automaticaly Adds Button After Description In Single Product View', WCQB_TXT ),
			'id' => WCQB_DB.'sp_qb_form_add',
			'type' => 'select', 
			'class' =>'chosen_select',
			'options' => array('true' => __("Yes",WCQB_TXT) , 'false' => __("No (I Will Use Shortcode)",WCQB_TXT) )
		);	
		$settings_array[] = array(
			'name' => __( 'Quick Buy Form Position', WCQB_TXT ),
			'desc_tip' => __( 'Where the button need to be added in single page .. before / after',WCQB_TXT ),
			'id' => WCQB_DB.'sp_qb_form_position',
			'type' => 'select', 
			'class' =>'chosen_select',
			'options' => $this->show_position
		);	*/ 
		...
		$settings_array[] = array(
			'name' => __( 'Quick Buy Form Heading',WCQB_TXT ),
			'desc_tip' => __( 'You can add your heading text', WCQB_TXT ),
			'id' => WCQB_DB.'sp_qb_form_heading',
			'type' => 'text', 
		);

	    $settings_array[] = array(
			'name' => __( 'Quick Buy Form Button Text',WCQB_TXT ),
			'desc_tip' => __( 'You can add your button text', WCQB_TXT ),
			'id' => WCQB_DB.'sp_qb_form_btn_text',
			'type' => 'text', 
		);

	
		$settings_array[] = array( 'type' => 'sectionend', 'id' => WCQB_DB.'quick_buy_form'  );
		$settings_array[] = array('name' => __( 'Shortcode', WCQB_TXT ),
								  'type' => 'title', 
								  'desc' => __( 'Place shortcode in product single page [wc_quick_buy_form]',WCQB_TXT ), 
      							  'id' => WCQB_DB.'quick_buy_form' );

		$settings_array[] = array('name' => __( 'Shortcode Use in Other Page', WCQB_TXT ),
								  'type' => 'title', 
								  'desc' => __( 'Place shortcode and write product id [wc_quick_buy_form product=1]',WCQB_TXT ), 
      							  'id' => WCQB_DB.'quick_buy_form' );

		return apply_filters('woocommerce_quick_buy_form_position_settings',$settings_array);
	}
